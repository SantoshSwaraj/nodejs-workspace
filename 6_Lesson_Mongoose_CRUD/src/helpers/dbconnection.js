...
const DBURL = require('../conf/db.conf').DBURL;
...
module.exports = mongoose.connect(DBURL,options,(err)=>{
    if(err)
...
db.on('error',console.error.bind(console,'Connection Erorr: Connection Failed!'));

db.once('open',()=>{
    console.log(`Mongoose Connected Successfully on : ${DBURL}`);
});
